...
<html>
    <head>
        ...
        * {
        ...
        #display {
            display: grid;
            grid-template-rows: 6em 40em 3em;
            ...
            ?>
                ...
                <div id="final">
                    <form action="../order/cancel.php" method="POST">
                       <input type="submit" id="back" value="Cancel">
                    </form>
...
<?php 
    ...
    function displayInfo($orderId) {
        ...
        if(isset($_POST["firstName"]) and isset($_POST["lastName"]) and isset($_POST["phoneNum"])) {
            $firstName = $_POST["firstName"];
            $lastName = $_POST["lastName"];
            $phoneNum = $_POST["phoneNum"];
    
            $sql = "INSERT INTO order_contact_info(
                        order_id,
                        first_name,
                        last_name,
                        phone_number
        ...
        if ($serviceMode == 1) {
            ...
            if (isset($_POST["suit"]) and isset($_POST["street"])
                and isset($_POST["city"]) and isset($_POST["country"])
                and isset($_POST["postalCode"])) {
                    ...
                    $sql = "INSERT INTO order_deliver_address (
                                order_id,
                                suit_number,
                                street,
                                city,
        ...
        $contaction = $contactResult->fetch_array(MYSQLI_ASSOC);

        echo        '<div class="personalInfo" id="infoTitle">Delivery Info: </div>
                    <div class="personalInfo">Service Method: ' . interpretServiceMode($serviceMode) . '</div>
                    <div class="personalInfo" id="name"> Name: 
                    ' . $contaction['first_name'] . ' ' . $contaction['last_name'] .'
                    </div>
                    <div class="personalInfo" id="phoneNum"> Phone Number: 
                    ' . $contaction['phone_number'] . '
                    </div>';
        ...
        if($serviceMode == 1) {
            ...
            $addressResult = $conn->query($sql);
            $address = $addressResult->fetch_array(MYSQLI_ASSOC);
            
            
            echo   '<div class="personalInfo" id="addressTitle">Address: </div>
                    <div class="personalInfo" id="municipal"> 
                        ' . $address['suit_number'] . '-' . $address['street'] .'
                    </div>
                    <div class="personalInfo" id="broadAddress">
                        ' . $address['city'] . ', ' . $address['country'] . '
                    </div>
                    <div class="personalInfo" id="postalCode">
                        ' . $address['postal_code'] . '
                    </div>';
        }
    ...
    function interpretServiceMode($serviceMode) {
        if ($serviceMode == 0){
            return "Dine In";
        }
        else {
